...
const encrypt = (key, plaintext, ivSeed) => {
  if (typeof key !== 'string' || !key) {
    throw new AES256Error('Key must be a non-empty string', {
      error: {
        code: 'ENOENCRYPTKEY'
      }
    })
  ...
  if (typeof plaintext !== 'string' || !plaintext) {
    throw new AES256Error('Plaintext value must be a non-empty string', {
      error: {
...
const decrypt = (key, encrypted) => {
  ...
  if (typeof encrypted !== 'string' || !encrypted) {
    throw new AES256Error('Encrypted value must be a non-empty string', {
      error: {
